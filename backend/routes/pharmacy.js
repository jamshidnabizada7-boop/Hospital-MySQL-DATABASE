/**
 * routes/pharmacy.js — Full CRUD: Medicines, Inventory, Pharmacy locations
 */
...
router.get('/medicines/:id', authenticate, async (req, res) => {
  try {
    const [rows] = await db.query(`
      ...
      JOIN Medicine_Category mc ON m.Category_ID=mc.Category_ID
      WHERE m.Medicine_ID=?`, [req.params.id]);
...
router.put('/medicines/:id', authenticate, async (req, res) => {
  const { category_id, medicine_name, generic_name, manufacturer,
          dosage_form, strength, unit_price, requires_rx, is_active } = req.body;
  try {
    await db.query(`
      UPDATE Medicine SET Category_ID=?,Medicine_Name=?,Generic_Name=?,Manufacturer=?,
        Dosage_Form=?,Strength=?,Unit_Price=?,Requires_Rx=?,Is_Active=?
      WHERE Medicine_ID=?`,
      [category_id, medicine_name, generic_name||'', manufacturer||'',
       dosage_form||'Tablet', strength, unit_price||0, requires_rx||1,
...
router.post('/locations', authenticate, async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ success: true, pharmacy_id: result.insertId, message: 'Pharmacy added' });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.put('/locations/:id', authenticate, async (req, res) => {
  ...
  try {
    await db.query(
      'UPDATE Pharmacy SET Pharmacy_Name=?,Location=?,Phone=?,Is_Active=? WHERE Pharmacy_ID=?',
...
// POST /api/pharmacy/inventory — add new inventory record
router.post('/inventory', authenticate, async (req, res) => {
  ...
  try {
    const [result] = await db.query(`
      ...
      VALUES(?,?,?,?,?,?,?)`,
      [pharmacy_id, medicine_id, quantity||0, reorder_level||10,
       batch_number||'', expiry_date, unit_cost||0]);
    res.status(201).json({ success: true, inventory_id: result.insertId, message: 'Inventory record added' });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});

// PUT /api/pharmacy/inventory/:id/stock — update quantity
router.put('/inventory/:id/stock', authenticate, async (req, res) => {
  ...
  try {
    ...
    const [[inv]] = await conn.query(
      'SELECT Quantity_In_Stock FROM Inventory WHERE Inventory_ID=? FOR UPDATE', [req.params.id]);
...
// PUT /api/pharmacy/inventory/:id — full update
router.put('/inventory/:id', authenticate, async (req, res) => {
  const { quantity, reorder_level, batch_number, expiry_date, unit_cost } = req.body;
  try {
    await db.query(`
      UPDATE Inventory SET Quantity_In_Stock=?,Reorder_Level=?,Batch_Number=?,Expiry_Date=?,Unit_Cost=?
      WHERE Inventory_ID=?`,
      [quantity, reorder_level, batch_number, expiry_date, unit_cost, req.params.id]);
